...
class ProcessPanel(ttk.LabelFrame):
    ...
    def setup_ui(self):
        ...
        self.app.p_unit.grid(row=0, column=2, padx=(0, 10), sticky="w")
        ...
        self.app.t_unit.grid(row=0, column=5, sticky="w")
        ...
        self.app.flow_unit.grid(row=1, column=2, padx=(0, 10), sticky="w")
        ...
        self.app.flow_type.grid(row=1, column=4, columnspan=2, sticky="w")
        self.app.flow_type.bind("<<ComboboxSelected>>", self.app.update_ui_visibility)

        # Row 2: Thermodynamic model
        ttk.Label(grid, text=t("thermo_model")).grid(row=2, column=0, sticky="w", pady=6)
        self.app.thermo_model = ttk.Combobox(
            grid,
            values=[
        ...
        self.app.thermo_model.grid(row=2, column=1, columnspan=5, sticky="w", padx=(4, 4))
        ...
        for label_text, val in targets:
            btn = ttk.Button(
                seg_frame,
                text=label_text,
                style="SegBtnActive.TButton" if val == self.app.calc_target.get() else "SegBtn.TButton",
